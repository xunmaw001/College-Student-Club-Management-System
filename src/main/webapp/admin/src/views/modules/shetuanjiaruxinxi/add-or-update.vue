<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="优势" prop="youshi">
          <el-input v-model="ruleForm.youshi" 
              placeholder="优势" clearable  :readonly="ro.youshi"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="优势" prop="youshi">
              <el-input v-model="ruleForm.youshi" 
                placeholder="优势" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
...
export default {
  ...
  methods: {
	...
	this.ruleForm.shenqingliyou = this.ruleForm.shenqingliyou.replace(reg,'../../../ssm72axv/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `shetuanjiaruxinxi/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.shetuanjiaruxinxiCrossAddOrUpdateFlag = false;
                  this.parent.search();
